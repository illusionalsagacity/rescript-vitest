...
describe("Expect", () => {
  ...
  /*
  test("toBeSomeValue", _t => {
    Some(1)->expect->Expect.toBeSomeValue(1)
    Some(2.)->expect->Expect.toBeSomeValue(2.)
    Some("one")->expect->Expect.toBeSomeValue("one")
    Some(true)->expect->Expect.toBeSomeValue(true)
    Some({"foo": "bar"})->expect->Expect.toBeSomeValue({"foo": "bar"})

    let date = Js.Date.fromString("2021-01-01")
    Some(date)->expect->Expect.toBeSomeValue(date)
  })
  */
  ...
  test("toThrowError", _t => {
    // this should also accept a regex
